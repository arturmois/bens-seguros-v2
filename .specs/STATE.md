...
| AD-003 | E-mail sai só pelo job `email.send`, com payload serializável `{ template, to, props }`; `template` é uma chave de `src/emails/templates.ts`, que valida as props com Zod e devolve assunto + elemento React no worker | active | `.specs/features/auth-core/plan.md` (Landing 3) |
| AD-004 | Checagem de `Origin` própria em todo método mutável de `/api/*` (403 `ORIGIN_NOT_ALLOWED`), além da do Better Auth. Webhooks (Fase 5+) entram como exceção explícita quando existirem | active | `.specs/features/auth-core/plan.md` (Landing 7) |
