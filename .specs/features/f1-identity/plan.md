...
| stored data | a migration converte `OWNER` em `ADMIN` e `VIEWER` em `COMMERCIAL` em `Member` e `Invitation`, apaga o índice `Member_one_owner` e recria o enum. Produção não tem dados; o staging ainda não foi publicado; os bancos de dev convertem |
| stored data | `publicChatKey` é preenchida para as organizações existentes na mesma migration, antes de virar `NOT NULL` e única |
...
| --- | --- | --- |
| 1. Papéis do MVP | `enum Role { ADMIN MANAGER COMMERCIAL }`. Migration `mvp_roles`: `DROP INDEX "Member_one_owner"`; `UPDATE` de `OWNER`→`ADMIN` e `VIEWER`→`COMMERCIAL` em `Member` e `Invitation`; recria o tipo (`Role_new`, `ALTER COLUMN … TYPE … USING role::text::"Role_new"`, `DROP TYPE`, `RENAME`) | manter os cinco e só esconder dois no web: cada permissão nova seria decidida para papéis sem uso, e o ADR-016 aceito diz o contrário; `VIEWER`→`MANAGER`: daria visão da operação inteira a quem só podia ler |
| 2. Chave pública do link | `Organization.publicChatKey`: `NOT NULL`, `UNIQUE` global (a `Organization` não tem `organizationId`), 32 caracteres hex minúsculos (`randomBytes(16).toString('hex')` no server; o backfill usa `replace(gen_random_uuid()::text, '-', '')`). Link `/c/<publicChatKey>` | `/c/<slug>`: bonito, mas não pode ser rotacionado sem quebrar o slug, e o ADR-014 escolheu a chave; `channelId`/`organizationId` no link: expõe identificador interno (ADR-014) |
...
3. WHEN a migration `mvp_roles` roda sobre um banco com `Member` e `Invitation` em `OWNER` e `VIEWER` THEN the system SHALL deixar esses registros em `ADMIN` e `COMMERCIAL`, respectivamente, e o índice `Member_one_owner` SHALL deixar de existir
...
11. WHEN a migration roda sobre organizações existentes THEN cada uma SHALL receber um `publicChatKey` de 32 caracteres hex, todos distintos
